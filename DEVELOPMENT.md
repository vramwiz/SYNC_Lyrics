...
- フォームは画面状態と操作を担当し、波形描画、データ変換、同期計算など独立して説明できる処理は
  専用ユニットへ分ける。新機能を足す前に、既存ユニットへ置く責務か確認する。
- 1ユニットがおおむね1,000行を超える場合は、さらに増やす前に抽出できる責務を探す。
  行数を減らすだけの機械的な分割や、相互依存する断片への分割は行わない。
- 1フォルダーに `.pas` が10個を超える場合は、表示・同期・データ・形式などの責務で
  サブフォルダーを設ける。フォームの `.pas` と `.dfm` は同じフォルダーに置く。
- 移動時は `.dpr`、`.dproj`、関連テストの参照パスを合わせて更新し、DebugとReleaseをビルドする。
