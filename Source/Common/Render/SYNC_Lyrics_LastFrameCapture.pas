...
begin
  ...
  try
    try
      ...
      begin
        SetCaptureError('Framebuffer callback is unavailable.');
        Exit;
      end;
      SourcePointer := Video^.GetFramebufferTexture2D();
      if SourcePointer = nil then
      begin
        SetCaptureError('Framebuffer texture is unavailable.');
        Exit;
      end;
      ...
      CaptureStatus := Format('Framebuffer: %d x %d, DXGI format %d.',
